    ...
    while (fgets(buff,MAXRNXLEN,fp)) {
        ...
        if (i==0) {
            ...
            if (ver>=3.0||sys==SYS_GAL||sys==SYS_QZS) { /* ver.3 or GAL/QZS */
                strncpy(id,buff,3);
                // printf("---before :id in rinex L1211--- %s \n", id);
                sat=satid2no(id);
                sp=4;

                // printf("---before :sys in rinex L1211--- %d \n", sys);
                // printf("---before :sat in rinex L1211--- %d \n", sat);
                if (ver>=3.0) sys=satsys(sat,NULL);
                // printf("---sys in rinex L1213--- %d \n", sys);
                // printf("---sat in rinex L1213--- %d \n", sat);
            }
            else {
                     ...
                     obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    switch (*type) {
        ...
        case 'B': return readrnxnav(fp,opt,ver,SYS_CMP,nav); /* extension */
        case 'C': return readrnxclk(fp,opt,index,nav);
    }
    // std::cout<<"nav.n -> "<< nav->n <<std::endl;
