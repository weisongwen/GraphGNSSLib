 ...
 * Head file for psr_doppler_car_rtk_dyna.cpp
...
#define pose_state_size 3 // x,y,z, (clk_gps,clk_BeiDou are removed by DD)
...
public:
    ...
    */
    bool setStateMemoryAndInitialize()
    {
        ...
        */
        ...
        /* assign memory to ambiguity state*/
        for(int i = 0; i < length;i++)
        {
            /* ambiguity set... */
            int comStateSize = pose_state_size + ar_state_num[i][0];
            state_array[i] = new double[comStateSize]; //
            
            for(int j = 0; j < comStateSize; j++)
            {
                state_array[i][j] = 0;
            }
        }
    ...
    bool addDopplerFactors(ceres::Problem& problem)
    {
        ...
        for(iterdopp = doppler_map.begin(); iterdopp != doppler_map.end();iterdopp++, i++)
        {
            ...
            if(iterdoppNext != doppler_map.end())
            {
                ...
                dopplerConstraint::dopplerDynaCostFunction* doppler_cost_function =
                dopplerConstraint::Create(v_x_i, v_y_i, v_z_i, delta_t,  var_vec,i,&state_array, &pose_parameter_blocks,  &ar_state_num);
                auto IDs = problem.AddResidualBlock(doppler_cost_function, loss_function, pose_parameter_blocks); 

                /* AutoDiffCostFunction-based cost function */
                ...
                */
                ...
                if(findMasterSatellite(sat_id, closest_gnss_data, u_master_sv, u_iSV))
                {
                    ...
                    { 
                        /* DynamicAutoDiffCostFunction-based cost function */
                        std::vector<double*> pose_parameter_blocks;
                        DDCarrierPhaseConstraint::DDCarrierPhaseDynaCostFunction* DD_car_cost_function =
                        DDCarrierPhaseConstraint::Create(DD_measurement, base_pose,k,carrier_phase_index,&state_array, &pose_parameter_blocks,  &ar_state_num);
                        auto IDs = problem.AddResidualBlock(DD_car_cost_function, loss_function, pose_parameter_blocks); 
    ...
    /* get covariance matrix of latest epoch */
    bool getCovarianceMatrixOfLatestEpoch(ceres::Problem& problem)
    {
        ...
        cov_options.null_space_rank = 10;
        ...
        covariance.Compute(cov_block, &problem);
        covariance.GetCovarianceBlock(state_array[length-1],state_array[length-1], covMatrix.data());
    ...
    bool solveAmbiguityResolutionViaCovPropogation()
    {
        ...
        */
        ...
        for(int i = 0; i < n; i++)
        {
            ...
            a[i] = state_array[length-1][pose_state_size + i];
        }
        ...
        for(int i=0; i<AR_cnt; i++)
        {
            ...
            float_b(i) = state_array[length-1][pose_state_size + i];
            integer_b(i) = F[i];
        }
     ...
     /* solve the ambiguity resolution probelm 
      * (covariance from Ceres-solver) 
      * Theoretically, the performance should be the same as the covariance
      * propogation
      * */
