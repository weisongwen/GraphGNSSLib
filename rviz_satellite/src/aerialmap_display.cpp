...
void AerialMapDisplay::assembleScene()
{
  ...
  for (int xx = area.leftTop.x; xx <= area.rightBottom.x; ++xx)
  {
    for (int yy = area.leftTop.y; yy <= area.rightBottom.y; ++yy)
    {
      ...
      double const x = (xx - lastTileId_->coord.x) * tile_w_h_m * 1;
      double const y = -(yy - lastTileId_->coord.y) * tile_w_h_m * 1;
...
void AerialMapDisplay::transformAerialMap()
{
  ...
  // add by weisong
  double offsetX = 0; //-0.1
  double offsetY = 1;

  std::cout << "/* originOffsetX */" << originOffsetX<<std::endl;
  std::cout << "/* originOffsetY */" << originOffsetY<<std::endl;
